...
class DigitClassifier {
  ...
  static func newInstance(modelPath: String = defaultModelPath(),
                          completion: @escaping ((Result<DigitClassifier>) -> ())) {
  ...
  func classify(image: UIImage, completion: @escaping ((Result<(String, Float)>) -> ())) {
    DispatchQueue.global(qos: .background).async {
      ...
      DispatchQueue.main.async {
        completion(.success((humanReadableResult, Float(maxConfidence))))
      }
    }
  }
}
